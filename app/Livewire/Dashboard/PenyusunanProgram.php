...
#[Layout('layouts.app')]
class PenyusunanProgram extends Component
{
    ...
    public function mount()
    {
        ...
        Session::put('current_bidang', 'penyusunan_program');
    }
    ...
    public function deleteArsip()
    {
        try {
            ...
            $bidangSlug = 'penyusunan_program';
            
            // Validasi Bidang
            if ($arsip->bidang !== $bidangSlug) {
                session()->flash('error', 'Arsip ini tidak termasuk dalam Sub.Bagian Penyusunan Program.');
    ...
    {
        $pageTitle = 'Sub.Bagian Penyusunan Program';
        if (Auth::user() && Auth::user()->role !== 'super_admin') {
            $pageTitle = 'Selamat datang, Admin Program';
        }

        // Mengganti passing data activityData menjadi chartData
        return view('livewire.dashboard.penyusunan-program', $this->getArsipData())
            ->title($pageTitle);
    }
}